...
vtkStandardNewMacro(ttkMergeTreeRemake);

ttkMergeTreeRemake::ttkMergeTreeRemake() {
...
int ttkMergeTreeRemake::FillInputPortInformation(int port, vtkInformation *info) {
  if(port == 0) {
...
int ttkMergeTreeRemake::RequestData(vtkInformation *,
  ...
  // Order Array
  auto orderArray = this->GetOrderArray(input, 0, triangulation);
  ...
  std::vector<const ttk::mt::Propagation<ttk::SimplexId> *> sortedPropagations;
  {
    ...
    ttkTypeMacroT(
      ...
      (status = this->computeMergeTreeRemakeSegmentation<ttk::SimplexId, T0>(
