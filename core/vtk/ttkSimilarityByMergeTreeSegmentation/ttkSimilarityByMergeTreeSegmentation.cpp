...
int ttkSimilarityByMergeTreeSegmentation::RequestData(vtkInformation *,
  ...
  auto mergeTree = vtkDataObject::GetData(inputVector[0]);
  auto segmentation = vtkDataObject::GetData(inputVector[1]);

  auto iterationInformation = vtkDoubleArray::SafeDownCast(mergeTree->GetFieldData()->GetArray("_ttk_IterationInfo"));

  std::cout<<"xxx"<<std::endl;

  auto packedInput = vtkSmartPointer<vtkMultiBlockDataSet>::New();
  packedInput->SetBlock(0,mergeTree);
  packedInput->SetBlock(1,segmentation);

  if(iterationInformation->GetValue(0)>0){
    if(!this->ComputeSimilarityMatrix(output,
      this->PreviousInputs,
      packedInput
    ))
        return 0;
  ...
  this->PreviousInputs = vtkSmartPointer<vtkDataObject>::Take(packedInput->NewInstance());
  this->PreviousInputs->DeepCopy(packedInput);
...
int ttkSimilarityByMergeTreeSegmentation::ComputeSimilarityMatrix(
  ...
  auto m0 = vtkDataSet::SafeDownCast(inputsAsMB0->GetBlock(0));
  auto m1 = vtkDataSet::SafeDownCast(inputsAsMB1->GetBlock(0));

  auto d0 = vtkDataSet::SafeDownCast(inputsAsMB0->GetBlock(1));
  auto d1 = vtkDataSet::SafeDownCast(inputsAsMB1->GetBlock(1));


  const int nNodes0 = m0->GetNumberOfPoints();
  const int nNodes1 = m1->GetNumberOfPoints();
  std::cout<<nNodes0<< " "<<nNodes1<<std::endl;
